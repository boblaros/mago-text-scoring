...
function buildLocalPreflightResponse(): LocalUploadPreflightResponse {
  return {
    ready: true,
...
async function advanceToLocalValidate(user: ReturnType<typeof userEvent.setup>, withConfig = false) {
  await user.click(screen.getByTestId("upload-source-local"));
  await user.click(
    screen.getByTestId(withConfig ? "upload-config-uploaded" : "upload-config-generated"),
  );
...
describe("ModelUploadWizard", () => {
  ...
  it("branches into the local uploaded-config path", async () => {
    const { user } = renderWizard();

    await user.click(screen.getByTestId("upload-source-local"));
    await user.click(screen.getByTestId("upload-config-uploaded"));
    await user.click(screen.getByRole("button", { name: "Continue" }));

    expect(screen.getAllByText("Local upload with existing config").length).toBeGreaterThan(0);
